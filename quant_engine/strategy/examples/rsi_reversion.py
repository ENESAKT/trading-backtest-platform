...
class RsiReversion(BaseStrategy):
    """RSI Mean Reversion — Aşırı alım/satım dönüş stratejisi."""

    name = "rsi_reversion"
    description = "RSI aşırı alım/satım bölgelerinden dönüş stratejisi"
    version = "1.0"
    ...
    def generate_signals(
        ...
        # Aşırı satım → AL
        if current_rsi < oversold and not position.is_open:
            return 1

        # Aşırı alım → SAT
        if current_rsi > overbought and position.is_open:
            return -1
