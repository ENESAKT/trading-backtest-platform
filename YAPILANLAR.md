...
│   ├── backtest/
│   │   └── engine.py           # Lookahead-free backtest motoru ⚠️ dokunma listesi
│   ├── data/
│   │   ├── live_feed.py        # LiveDataService, PaperTradingRecorder ⚠️
│   │   ├── providers/
│   │   │   ├── binance_provider.py  # ⚠️ dokunma listesi
│   │   │   └── yfinance_provider.py # ⚠️ dokunma listesi
│   │   └── models.py
│   ├── research/
...
| `/api/v2/candles` | GET | Cache-aside OHLCV (SQLite miss → provider) |
...
| `/api/mali-analiz/{symbol}/reports` | GET | Finansal raporlar (metadata-only) |
| `/api/mali-analiz/{symbol}/events` | GET | KAP olayları (metadata-only) |
| `/api/mali-analiz/{symbol}/metric-history` | GET | Metrik geçmişi (metadata-only) |
...
**Middleware:**
- `CORSMiddleware` — `allow_origins=["*"]` ⚠️ production'da kısıtlanmalı
- `APIKeyMiddleware` — `API_KEY` env varsa `X-API-Key` header zorunlu
...
- `service.py`: `FinancialAnalysisService` — metadata-only, sahte veri üretmez
- `cache.py`: `FinancialAnalysisCache` — günlük SQLite cache
- `symbols.py`: `SYMBOL_METADATA`, `normalize_symbol` — sembol normalize
- Durum: Metadata/boş kontrat v1 hazır; gerçek KAP/provider bağlantısı yok
...
### 4.1 `quant_engine/backtest/engine.py` ⚠️
