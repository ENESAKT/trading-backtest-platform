...
- [x] Commit 4: İndikatör, temel teknik analiz ve ileri düzey teknik analiz OCR çıktılarından seçilen özellikler.
- [x] Commit 5a: Fuat Akman Sistem Trading OCR ve yardımcı dokümanlarından seçilen özellikler.
- [x] Commit 5b: VİOP/Vadeli eğitimlerinin OCR çıktılarından seçilen özellikler.
...
### 21.9 İlk Derin Okuma Çıktısı - İndikatörlerin Seçimi ve Kullanımı

Kaynak: `artifacts/borfin_indikator_ocr/ocr_report.md`, kurs içi `islem_hacmi_indikatorleri.doc`, `yon_ve_guc_analiz_eden_indikatorler.doc`, `hareketin_gucu_ile_ilgilenen_indikatorler.doc` ve `borfin_ind._siteye_eklenen.xls`. Yöntem: macOS Vision ile aralıklı kare OCR ve yardımcı dosya metin çıkarımı; konuşma içinde kalıp ekranda görünmeyen detaylar eksik olabilir.

- [ ] **İndikatör Taksonomi Motoru:** İndikatörler yalnızca alfabetik listelenmeyecek; trend, momentum, volatilite/bant, hacim, yön/güç, hareket gücü, fiyat dönüşü ve filtre kategorileriyle metadata taşıyacak. Problem: kullanıcı çok sayıda indikatör içinde hangisinin hangi problemi çözdüğünü seçemiyor. Ürün noktası: İndikatör Merkezi v2 + Eğitimler araması. Test: Bollinger volatilite/bant, OBV hacim, ADX yön/güç, RSI momentum kategorisinde bulunur.
- [ ] **Parametre Sözlüğü ve Guardrail:** Period, veri kaynağı, ortalama türü, standart sapma, momentum periodu, çizgi kalınlığı, panel/overlay, ölçek birleştirme ve alarm seviyeleri standart parametre şemasına bağlanacak. Problem: her indikatör farklı ayar diliyle kullanıcıyı kaybediyor. Ürün noktası: indikatör ayar drawer'ı ve StrategySpec validation. Test: aynı parametre şeması hem grafikte hem backtest preset'inde kullanılabilir.
- [ ] **Bollinger Eğitimden Grafiğe Akışı:** Bollinger için üst/orta/alt bant, period, standart sapma, veri kaynağı ve ortalama tipi açıklaması grafikteki örnekle bağlanacak; kullanıcı tek tıkla bant teması, band dışına taşma, orta banda dönüş veya bant genişliği stratejisi oluşturabilecek. Problem: kullanıcı Bollinger'i görüyor ama hangi kuralın test edilebilir olduğunu ayıramıyor. Ürün noktası: Eğitim konu sayfası -> grafik overlay -> backtest preset. Test: `BOL(20,2)` ayarı grafikte ve StrategySpec'te aynı parametrelerle oluşur.
- [ ] **Hacim Onay Paketi:** A/D, Chaikin, CMF, MFI, OBV, OBVx, VPT, NVI/PVI, Volume Oscillator ve Volume ROC ailesi "fiyat hareketini hacim teyit ediyor mu?" sorusuna göre gruplanacak. Problem: kullanıcı hacim indikatörlerini ayrı ayrı öğreniyor ama sinyal filtresi olarak kullanamıyor. Ürün noktası: Strategy Lab filtre blokları ve Explorer taraması. Test: fiyat kırılımı + OBV/VPT pozitif teyit koşulu sembol taramasında çalışır.
- [ ] **Yön ve Güç Filtre Paketi:** ADX/ADXR, DI/DX/DISpread, VHF, Mass Index, CCI ve Aroon ailesi giriş sinyali üretmekten çok rejim/güç filtresi olarak sunulacak. Problem: trend piyasası ile yatay piyasa aynı strateji kurallarıyla test ediliyor. Ürün noktası: kural kurucu `Trend/Güç Filtresi` bölümü. Test: ADX eşiği altında trend-following girişleri engellenir ve raporda "filtreledi" sayacı görünür.
- [ ] **Momentum/Oscillator Ailesi:** RSI/RMI, Stochastic Fast/Slow, Stochastic Momentum Index, Stochastic RSI, CMO, Momentum, Price ROC, Williams ve Ultimate Oscillator aşırı alım-satım, kesişim, uyumsuzluk ve orta çizgi davranışı aksiyonlarına ayrılacak. Problem: oscillator sinyalleri birbirine karışıyor ve her koşul al/sat gibi yorumlanıyor. Ürün noktası: indikatör kartı + StrategySpec blok önerileri. Test: RSI eşik dönüşü ve Stochastic kesişim presetleri farklı kural açıklaması üretir.
- [ ] **Overlay ve Ölçek Yönetimi:** Her indikatör "fiyat üstü çizilir", "alt panel ister", "fiyat ölçeğiyle birleştirilebilir" veya "ayrı ölçek gerektirir" metadata'sı taşıyacak. Problem: çoklu indikatör eklendiğinde grafik okunmaz hale geliyor. Ürün noktası: ChartPanel indikatör yerleşimi. Test: Bollinger fiyat paneline, RSI alt panele, OBV ayrı ölçeğe otomatik yerleşir.
- [ ] **Görsel Örnek Galerisi:** Kurs ekran görüntüleri kopyalanmadan PiyasaPilot-native mini grafik örnekleri üretilecek; her indikatör kartında "grafikte nasıl görünür?" örneği olacak. Problem: metinsel açıklama tek başına indikatör davranışını anlatmıyor. Ürün noktası: Eğitimler sekmesi ve indikatör drawer'ı. Test: örnekler telifli ekran görüntüsü içermeden sentetik veriyle render edilir.
- [ ] **Repaint/Gecikme Risk Etiketleri:** ZigZag, Heiken Ashi, Ichimoku ofsetleri, smoothing kullanan ortalamalar, Parabolic SAR dönüşleri ve oscillator false-signal riskleri açık etiketlerle gösterilecek. Problem: kullanıcı grafikte sonradan güzel görünen çizgiyi canlı işlemde aynı sanıyor. Ürün noktası: indikatör kartı, backtest raporu ve preset uyarısı. Test: repaint/gecikme etiketi olan preset raporda risk uyarısı üretir.
- [ ] **İndikatörden StrategySpec'e Sihirbaz:** Seçilen indikatöre göre kural şablonları önerilecek: Bollinger dönüş/kırılım, ADX filtreli trend, RSI/RMI eşik dönüşü, Stochastic kesişim, OBV/VPT hacim teyidi, Parabolic SAR trailing çıkış. Problem: eğitim bilgisi grafikte kalıyor, test edilebilir sisteme dönüşmüyor. Ürün noktası: Eğitim konu sayfası ve Strategy Lab `Kural` sekmesi. Test: her şablon JSON StrategySpec olarak kaydedilir ve backtest marker'ı üretir.

### 21.10 İlk Derin Okuma Çıktısı - Yaşar Erdinç Temel Teknik Analiz

Kaynak: `artifacts/borfin_teknik_analiz_yasar_ocr/ocr_report.md`, `TEKNIK_ANALIZ_BASLANGIC_ve_GELISTIRMA_SUNUM.pptx` ve `ACTIVE_CHARTS_TANITIM.docx`. Yöntem: macOS Vision ile aralıklı kare OCR ve yardımcı dosya metin çıkarımı; konuşma içinde kalıp ekranda görünmeyen detaylar eksik olabilir.

- [ ] **Teknik Analiz Kanıt Katmanı:** Trend, kanal, Fibonacci, formasyon, mum ve gap fikirleri tek başına sinyal değil; her analizde zaman aralığı, teyit, stop, hedef ve backtest durumu zorunlu metadata olacak. Problem: kullanıcı çizimi tahmin gibi okuyabiliyor. Ürün noktası: Teknik Analiz Labı ve backtest raporu. Test: stop/hedef veya teyit bilgisi olmayan analiz "taslak" kalır.
- [ ] **Çizimden Kurala Dönüştürme:** Trend çizgisi, yatay destek/direnç, kanal çizgisi, Fibonacci seviyesi ve hız direnç çizgisi kırılım/temas/retest koşuluna çevrilebilecek. Problem: çizimler grafikte kalıyor, alarm veya test üretmiyor. Ürün noktası: ChartPanel drawing tools + StrategySpec taslağı. Test: kullanıcı trend çizgisi çizer, "kırılırsa uyar" ve "geçmişte test et" aksiyonları oluşur.
- [ ] **Kanal Stratejisi Labı:** Kanal üst/alt bandı, orta çizgi, hedef T1/T2/T3, hacim ve risk/ödül birlikte takip edilecek. Problem: kanal analizi subjektif kalıyor. Ürün noktası: çizim özellik paneli ve backtest preset'i. Test: kanal alt temasından dönüş ve kanal üst hedefi kuralları marker üretir.
- [ ] **Mum Formasyon Sandbox'ı:** Yükseliş/düşüş mumları ve formüllü mum stratejileri hazır kopya değil, kullanıcı tarafından düzenlenebilir koşul seti olarak tanımlanacak. Problem: kullanıcı mum ismini biliyor ama kendi verisinde işe yarayıp yaramadığını ölçemiyor. Ürün noktası: Kural Lab `Mum` blokları. Test: tanımlı mum koşulu geçmiş veri üzerinde başarı oranı, işlem sayısı ve false-positive raporu verir.
- [ ] **Fibonacci Araç Zinciri:** Dip/tepe seçimiyle geri dönüş, uzatma, fan, zaman çizgisi, yay, quadrant ve hız direnç seviyeleri üretilecek; seviyeler destek/direnç adayı olarak alarm ve backtest kuralına bağlanacak. Problem: Fibonacci çizimi manuel ve seçim hassasiyetine açık. Ürün noktası: Fibonacci drawing inspector. Test: dip/tepe değişince seviyeler ve bağlı alarmlar yeniden hesaplanır; rapor seçim hassasiyeti uyarısı verir.
- [ ] **Formasyon Workbench:** OBO, üçgen, bayrak, elmas, çift tepe/dip, fincan-kulp ve çanak formasyonları için boyun/kırılım çizgisi, hedef, geçersizleşme ve hacim teyidi alanları olacak. Problem: formasyonlar geriye dönük güzel görünür ama işlem planına dönüşmez. Ürün noktası: Teknik Analiz Labı + eğitim drawer'ı. Test: OBO boyun çizgisi kırılımı StrategySpec koşulu ve hedef/stop önerisi üretir.
- [ ] **Gap Sınıflandırıcı:** Normal, kırılma, trend güçlendirici, tükeniş ve ada boşluğu ayrı etiketlenecek; trend bağlamı, hacim ve önceki kapanış-açılış farkı gösterilecek. Problem: her boşluk aynı sinyal gibi yorumlanıyor. Ürün noktası: grafik overlay ve risk uyarısı. Test: gap türü sınıflandırması raporda "devam", "dikkat" veya "tükeniş riski" etiketi verir.
- [ ] **Aykırılık Paneli:** Fiyat ile Stochastic, RSI, MFI, CCI, MACD veya kullanıcı göstergesi arasındaki pozitif/negatif aykırılık adayları çizgisel olarak gösterilecek; teyitsiz aykırılık tek başına al/sat sayılmayacak. Problem: aykırılık subjektif ve gecikmeli okunuyor. Ürün noktası: indikatör alt paneli + alarm. Test: iki fiyat tepesi ve iki gösterge tepesi seçilince negatif aykırılık adayı oluşur, teyit kırılımı olmadan backtest girişi açılmaz.
- [ ] **Güvenli Gösterge Geliştirme Sandbox'ı:** Kurs ekranlarında görülen endekse göre kıyaslama ve TKE benzeri kompozit gösterge fikri PiyasaPilot DSL içinde güvenli, telifsiz ve kullanıcı tanımlı formüllere dönüştürülecek. Problem: kullanıcı özel gösterge fikrini ham platform formülüyle taşımaya çalışıyor. Ürün noktası: Indicator Builder + StrategySpec. Test: sembol getirisi - endeks getirisi gibi basit bir custom indicator derlenir, grafikte çizilir ve güvenli ifade dışına çıkamaz.
- [ ] **Strateji Disiplin Journal'ı:** Son ders ekranlarında görülen strateji uygulama dosyası fikri uygulamaya alınacak: alış hedefi, satış hedefi, stop, lot, gün sayısı, K/Z, stop oldu mu, başarı oranı ve kazandırma katsayısı paper/backtest journal'ında tutulacak. Problem: kullanıcı stratejiyi yeterince uzun ve disiplinli takip etmiyor. Ürün noktası: Paper Journal + Postmortem. Test: en az 36 işlem tamamlanmadan strateji "kanıtlandı" etiketi alamaz.

### 21.11 İlk Derin Okuma Çıktısı - Yaşar Erdinç İleri Düzey Teknik Analiz

Kaynak: `artifacts/borfin_ileri_teknik_analiz_yasar_ocr/ocr_report.md`, `tumformuller.doc`, `sistemler.zip`, `IDTA_dosyalar.zip` ve `tradesistemmakro2.xls` string taraması. Yöntem: macOS Vision ile aralıklı kare OCR + yardımcı dosya metin/zip envanteri; konuşma içinde kalıp ekranda görünmeyen detaylar eksik olabilir. Formüller birebir taşınmayacak; yalnızca PiyasaPilot'a özgü kural aileleri ve iş akışları çıkarılacak.

- [ ] **Sistem Kataloğu ve Kural Ailesi Haritası:** 52 `.mst` sistem fikri ham formül olarak değil; trend kesişimi, momentum, oscillator, volatilite/ATR, breakout/HHV-LLV, kompozit/vote, göreceli güç, mum, Turtle ve Hull MA aileleri olarak kataloglanacak. Problem: kullanıcı çok sayıda sistem adını görür ama neyi test ettiğini anlayamaz. Ürün noktası: Eğitim presetleri + Strategy Lab. Test: her preset aile, risk etiketi, parametre alanı ve açıklama taşır.
- [ ] **Güvenli Formula DSL Builder:** Indicator Builder ve System Tester mantığı PiyasaPilot AST tabanlı DSL'e çevrilecek; izinli fonksiyonlar `cross`, `mov`, `ref`, `hhv`, `llv`, `roc`, `atr`, `rsi`, `pdi/mdi`, `sum`, `stdev` gibi beyaz listeyle sınırlanacak. Problem: ham platform formülü kopyalamak hem kırılgan hem lisans riski taşır. Ürün noktası: Indicator Builder + StrategySpec compiler. Test: tehlikeli veya desteklenmeyen ifade reddedilir; desteklenen ifade deterministik seri üretir.
- [ ] **AL/SAT Dörtlemesi Modeli:** Sistemler long entry, long exit, short entry ve short exit koşullarını ayrı alanlarda taşıyacak; BIST için short simülasyon etiketi zorunlu olacak. Problem: tek al/sat koşulu gerçek pozisyon yaşam döngüsünü anlatmıyor. Ürün noktası: StrategySpec schema ve backtest engine. Test: short kapalıyken `AÇIĞA SAT` bloğu yürütülmez ve raporda pasif nedeni görünür.
- [ ] **System Tester Ayar Kartı:** Başlangıç bakiyesi, komisyon, periyot, sembol evreni, işlem yönü, bar kapanışı ve optimizasyon değişkenleri raporun üstünde kilitli varsayım olarak gösterilecek. Problem: aynı sistem farklı ayarlarla bambaşka sonuç verir. Ürün noktası: backtest varsayım kartı. Test: varsayım değişince rapor hash'i ve karşılaştırma etiketi değişir.
- [ ] **Optimizasyon Koruma Katmanı:** `opt1/opt2/opt3` mantığı parametre aralığı, adım, test sayısı, stabil komşuluk, in-sample/out-of-sample ve walk-forward uyarılarıyla yönetilecek. Problem: en iyi parametre kolayca overfit olur. Ürün noktası: Parametre Deneyleri v2. Test: yüksek test sayısı ve dar en iyi nokta "kırılgan optimizasyon" uyarısı üretir.
- [ ] **Explorer / Sembol Tarayıcı v2:** Explorer ekranlarında görülen filtre ve kolon mantığı PiyasaPilot tarayıcısına bağlanacak; kullanıcı sistem filtresi, ek kolon, son sinyal ve gösterge değeriyle sembol arayacak. Problem: strateji tek grafikte deneniyor, evren taraması kopuk kalıyor. Ürün noktası: Explorer v3. Test: RSI aykırılık adayı veya HLMA sinyali sembol listesinde kolonlarla döner.
- [ ] **Expert Advisor ve Alarm Audit'i:** Expert Advisor/gerçekleşen sistem alarmları fikri, PiyasaPilot'ta alarm üretildiğinde hangi kural, sembol, bar, fiyat ve filtreyle oluştuğunu loglayacak. Problem: kullanıcı alarmın neden geldiğini sonradan izleyemiyor. Ürün noktası: alarm motoru + audit log. Test: alarm kaydı StrategySpec rule id ve parametre snapshot'ı içerir.
- [ ] **Performans Raporu v2:** System Tester sonuçlarında görülen overall, yüzde getiri karşılaştırması, pozisyonlar, ortalama kar/zarar, kazanan/kaybeden sayısı, en karlı ve en kötü pozisyon metrikleri PiyasaPilot raporunda standart olacak. Problem: kullanıcı sadece toplam getiriye bakıyor. Ürün noktası: Backtest report. Test: rapor en iyi/kötü işlem, ortalama kar/zarar ve işlem bar süresini üretir.
- [ ] **Çoklu Sistem Karşılaştırıcı:** Aynı sembol/timeframe üzerinde HLMA, RSI, TKE, TTFC, TSI, ATR ve Turtle gibi sistem aileleri yan yana koşup ortak metrikle kıyaslanacak. Problem: kullanıcı sistemleri ayrı ayrı deneyip sonuçları zihninde karşılaştırıyor. Ürün noktası: Strategy Lab karşılaştırma sekmesi. Test: iki preset aynı veri aralığında koşar ve ortak metrik tablosu üretir.
- [ ] **Sistem Pack Governance:** Borfin kaynaklı sistem fikirleri telifli formül/metin içermeyen PiyasaPilot preset pack'e dönüştürülecek; her preset kaynak-kavram, versiyon, parametre, test kapsamı ve risk etiketi taşıyacak. Problem: eğitim esinli presetler kontrolsüz büyür. Ürün noktası: Strategy Pack import/export. Test: preset pack JSON şeması doğrulanır ve eksik risk etiketiyle yayınlanamaz.

### 21.12 İlk Derin Okuma Çıktısı - Fuat Akman Sistem Trading ve Araçları

Kaynak: `artifacts/borfin_sistem_trading_fuat_ocr/ocr_report.md` ve `artifacts/borfin_sistem_trading_fuat_docs_text/`. Yöntem: macOS Vision ile aralıklı kare OCR + 19 `.doc` yardımcı dosyanın metin çıkarımı; konuşma içinde kalıp ekranda görünmeyen detaylar eksik olabilir. Eğitimdeki formüller birebir taşınmayacak; PiyasaPilot'a özgü güvenli kural modeli, tarayıcı ve rapor akışına dönüştürülecek.

- [ ] **StrategySpec Fonksiyon Sözlüğü:** `abs`, `correlation`, `cum`, tarih/saat fonksiyonları, `fml`, `ref`, `security data`, `sum`, `barssince`, `cross`, `highest/lowest`, `valuewhen`, `if` ve `trend` benzeri seri fonksiyonlar kullanıcıya açıklamalı, örnekli ve testli fonksiyon kataloğu olarak sunulacak. Problem: kullanıcı formül yazar ama fonksiyonun veri bağımlılığını ve gecikmesini bilmez. Ürün noktası: Formula DSL Builder + eğitim drawer'ı. Test: her fonksiyon fixture veriyle deterministik sonuç üretir.
- [ ] **No-Lookahead Kural Doğrulayıcı:** `ref`, `valuewhen`, `barssince`, `highest since` ve `lowest since` tarzı zamansal fonksiyonlar geleceğe bakma riski açısından doğrulanacak. Problem: formül doğru görünür ama backtestte geleceği sızdırabilir. Ürün noktası: StrategySpec compiler. Test: pozitif offset veya kapanmamış bar kullanan kural reddedilir ya da açık uyarı üretir.
- [ ] **AL/SAT/Short/Stop Editörü:** System Tester ekranındaki AL, SAT, açığa sat, açık pozisyon kapat, stop, not ve değişken alanları PiyasaPilot'ta ayrı kural sekmeleri olacak. Problem: tek koşullu strateji gerçek işlem yaşam döngüsünü açıklamıyor. Ürün noktası: Strategy Lab kural editörü. Test: her pozisyon yönü için ayrı koşul validasyonu ve raporda ayrı sinyal sayacı oluşur.
- [ ] **Simülasyon Varsayımları Kartı:** Simülasyon adı, başlangıç bakiyesi, kaldıraç, komisyon, faiz, işlem tipi, açık pozisyonları son barda kapatma, kalıcı sinyal modu, fiyat seçeneği ve anlık güncelleme ayarları backtest raporunun üstünde kilitli varsayım olarak gösterilecek. Problem: ayarlar görünmeden rapor yanıltıcı olur. Ürün noktası: Backtest report assumption card. Test: varsayım değişince rapor kimliği ve karşılaştırma etiketi değişir.
- [ ] **Optimizasyon Değişkenleri Paneli:** `OPT1...OPT10` mantığı PiyasaPilot'ta ad, alt/üst sınır, adım, varsayılan değer, test sayısı, kullanılan strateji ailesi ve overfit uyarısıyla yönetilecek. Problem: optimizasyon sayı üretir ama kullanıcı test patlamasını ve kırılganlığı görmez. Ürün noktası: Parametre Deneyleri v2. Test: 28/784 gibi yüksek kombinasyon sayılarında maliyet ve overfit uyarısı çıkar.
- [ ] **Sistem Raporu Drilldown'u:** Özet, işlemler, açık pozisyon, performans ve sistem bilgileri ayrı sekmeler olacak; başlangıç bakiyesi, komisyon, getiri, işlem sayısı, long/short dağılımı, ortalama kar/zarar, en iyi/kötü pozisyon, en uzun/kısa pozisyon, işlemsiz süre, son durum ve optimizasyon değerleri standart metrik olacak. Problem: toplam getiri strateji kalitesini tek başına anlatmaz. Ürün noktası: Backtest report v2. Test: örnek rapor tüm metrikleri ve trade drilldown linklerini üretir.
- [ ] **Grafik Üzerinde Göster ve Excel Yerine Canlı Bağlantı:** Raporun "grafik üzerinde göster" ve dışa aktarma fikri PiyasaPilot'ta rapor satırından ilgili bar/işlem marker'ına odaklanma ve CSV/JSON export olarak çalışacak. Problem: rapor tablosu grafikten kopuk kalıyor. Ürün noktası: Report -> Chart deep link. Test: en kötü pozisyon satırı tıklandığında grafik ilgili giriş/çıkış aralığına gider.
- [ ] **Örnek Sistemleri Preset Ailesine Çevirme:** 18 yardımcı sistem dosyası ham formül olarak saklanmayacak; RSI/ortalama, MOST, Bollinger bant yönü, Stochastic pencere, fiyat dönüşü, kompozit oscillator, VHF-volatilite, trend-kırılım ve valuewhen tabanlı yapı ailelerine ayrılacak. Problem: kullanıcı örnek sistemleri anlamadan kopyalar. Ürün noktası: Strategy preset pack. Test: her preset aile açıklaması, parametre alanı, risk etiketi ve telifsiz StrategySpec çıktısı taşır.
- [ ] **Explorer Kolon ve Filtre Builder'ı:** Explorer ekranlarındaki kolon ve filtre mantığı, PiyasaPilot tarayıcısında "son sinyalden kaç bar geçti", kısa getiri, ortalama, beklenen kırılım ve özel kolon üretimi olarak tasarlanacak. Problem: kullanıcı stratejiyi tek sembolde dener, fırsat evrenini tarayamaz. Ürün noktası: Explorer v3. Test: kolon filtresi sembol listesinde değer ve filtre nedeni döndürür.
- [ ] **Expert Advisor Alarm Audit'i:** Expert Advisor ve gerçekleşen alarm ekranları PiyasaPilot alarm motorunda kural id, sembol, bar zamanı, fiyat, gösterge değerleri, kullanıcı ayarı ve teslim durumu ile saklanacak. Problem: alarm geldikten sonra "neden geldi?" sorusu kayboluyor. Ürün noktası: Alarm center + audit log. Test: alarm geçmişi rule snapshot ve delivery status içerir.
- [ ] **Sistem Düşünsel Altyapı Checklist'i:** Kursun sistem trading yaklaşımı, strateji fikri, veri, kural, test, optimizasyon, rapor okuma, tarama ve alarmı tek akışa bağlayan checklist olarak kullanılacak. Problem: kullanıcı formül yazmayı sistem kurmak sanıyor. Ürün noktası: Strategy Lab onboarding checklist. Test: veri/maliyet/rapor/tarama adımı tamamlanmadan strateji "paper'a hazır" etiketi alamaz.

### 21.13 İlk Derin Okuma Çıktısı - VOB ve Vadeli Trade

Kaynak: `artifacts/borfin_vob_yasar_ocr/ocr_report.md` ve `artifacts/borfin_vadeli_trade_bolgun_ocr/ocr_report.md`. Yöntem: macOS Vision ile aralıklı kare OCR; konuşma içinde kalıp ekranda görünmeyen detaylar eksik olabilir. Bu bölüm spot backtestin üstüne "VİOP gerçekliği" katmanı eklemek için kullanılacak.

- [ ] **Kontrat Şeması ve Vade Zinciri:** VİOP sembolü dayanak varlık, kontrat çarpanı, fiyat kotasyonu, minimum fiyat adımı, vade ayı, yakın vade, uzlaşma, teorik fiyat, açık pozisyon ve sözleşme tipi metadata'sı taşıyacak. Problem: kullanıcı vadeli sembolü spot hisse gibi okuyor. Ürün noktası: VİOP provider + sembol detay paneli. Test: XU030 ve pay vadeli kontratları farklı çarpan/tick/vade şemasıyla doğrulanır.
- [ ] **Teminat/Kaldıraç/PnL Simülatörü:** VOB30 ekranlarında görülen teminat, pozisyon ve kaldıraç mantığı backtest varsayımına bağlanacak; kontrat adedi değişince nominal pozisyon, teminat kullanımı, kaldıraç ve yüzde teminat etkisi hesaplanacak. Problem: küçük fiyat değişimi teminat üzerinde büyük etki yaratır. Ürün noktası: VİOP risk kartı. Test: fiyat değişimi aynıyken kontrat adedi arttığında teminat etkisi doğrusal büyür.
- [ ] **Vade Yenileme ve Rollover Motoru:** Yakın vade/sonraki vade ayrımı, aynı anda işlem gören vade ayları ve iki ayda bir yenileme fikri backtestte açık rollover varsayımı olacak. Problem: sürekli kontrat grafiği gerçek işlem maliyetini saklar. Ürün noktası: VİOP backtest ayarı. Test: rollover açıkken rapor kontrat geçiş tarihini, fiyat farkını ve işlem maliyetini listeler.
- [ ] **Uzlaşma ve Mark-to-Market Günlüğü:** Vadeli pozisyonlarda günlük uzlaşma, nakit/teminat hareketi, açık pozisyon ve son durum ayrı günlükte izlenecek. Problem: vadeli PnL sadece kapanışta oluşuyormuş gibi algılanıyor. Ürün noktası: VİOP report ledger. Test: açık pozisyon taşındığında günlük nakit/teminat değişimi rapora yazılır.
- [ ] **Teorik Fiyat ve Baz Analizi:** Spot, vadeli, risksiz faiz, temettü ve taşıma maliyeti ilişkisi teorik fiyat-baz paneline çevrilecek. Problem: vadeli fiyatın spotun altında/üstünde olması tek başına sinyal sanılıyor. Ürün noktası: VİOP analiz paneli. Test: teorik fiyat veri yoksa panel uyarı verir; veri varsa spot-vadeli farkı ve baz etiketi üretir.
- [ ] **Kurumsal Aksiyon Uyarlama Katmanı:** Pay vadeli sözleşmelerinde temettü, bedelsiz/bedelli sermaye artırımı, standart olmayan sözleşme, baz fiyat ve çarpan değişimi takip edilecek. Problem: hisse vadeli backtestte çarpan ve baz fiyat uyarlaması yapılmazsa PnL bozulur. Ürün noktası: corporate action adjuster. Test: çarpan değişen örnek kontratta pozisyon değeri sürekliliği korunur.
- [ ] **VİOP Strateji Presetleri:** VOB eğitimindeki uzun vadeli long/short, TKE + 300 HO rejim filtresi, kısa vadeli EMA/MACD giriş, kademeli kar alma ve stop mantıkları ham eğitim kuralı olarak değil, VİOP'a özgü risk/teminat kontrollü preset aileleri olarak sunulacak. Problem: vadeli strateji spot strateji gibi kopyalanıyor. Ürün noktası: Strategy preset pack. Test: preset, kontrat varsayımı olmadan çalıştırılamaz.
- [ ] **Hedge/Spekülasyon/Arbitraj Modu:** Kullanıcı backtest amacını riskten korunma, spekülasyon veya arbitraj olarak seçecek; rapor metrikleri buna göre değişecek. Problem: aynı kontrat farklı amaçlarla tamamen farklı başarı ölçütü ister. Ürün noktası: Strategy Lab amaç seçici. Test: hedge modunda portföy beta/delta riski, spekülasyon modunda getiri/drawdown, arbitraj modunda baz kapanması öne çıkar.
- [ ] **VİOP Veri Gerçekliği Kapısı:** Lisanslı gerçek zamanlı veya geçmiş kontrat verisi yoksa sonuç "eğitim/sentetik" etiketiyle sunulacak; paper veya robot aksiyonları pasif kalacak. Problem: vadeli testte yanlış veri kaynağı çok pahalı hata üretir. Ürün noktası: provider status + activation guard. Test: VİOP provider `not_configured` iken raporda gerçek işlem önerisi çıkmaz.
- [ ] **Maliyet ve Likidite Varsayımları:** Komisyon, spread, slippage, emir kademesi, açık pozisyon ve hacim alanları VİOP backtestte zorunlu varsayım olacak. Problem: kontrat likiditesi ve spread maliyeti stratejiyi tersine çevirebilir. Ürün noktası: Backtest assumption card. Test: spread/slippage boşsa VİOP raporu kalite skorundan ceza alır.
- [ ] **Spot Karşılaştırmalı Performans:** VOB stratejilerinde görülen endeks karşılaştırma mantığı, spot endeks/hisse, vadeli kontrat ve nakit rezerv getirisiyle yan yana gösterilecek. Problem: kaldıraçlı getiri tek başına üstünlük sanılıyor. Ürün noktası: Performance comparison. Test: rapor aynı dönemde spot getiri, vadeli getiri, teminat getirisi ve nakit rezerv etkisini ayrı gösterir.
