...
- [ ] 3.2 `POST /api/backtest/run` endpoint (Python `BacktestEngine`).
- [ ] 3.3 Strateji blueprint formatı (parametre şeması + meta).
- [ ] 3.4 TS'te `StrategyPanel` → API'ye `POST` atıyor, equity eğrisini Chart.js ile çiziyor.
- [ ] 3.5 Live signal feed: `/ws/signals` (DecisionEngine her bar kapanışında çalışır).
- [ ] 3.6 TS'te `SignalFeed` panel.
- [ ] 3.7 Eski 4 strateji + yeni 4 strateji (toplam 8): EMA cross, RSI mean-rev, BB rev, breakout, **donchian breakout, MACD div, supertrend, mean-reversion VWAP**.
