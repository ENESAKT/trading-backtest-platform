...
- [x] Auth guard testleri eklendi; login cookie, admin gate ve Pro feature gate hedefli doğrulandı.
- [x] Mobil Bearer token login/refresh endpointleri eklendi; Flutter iskeleti auth, terminal, portfolio, settings ve onboarding ekranlarıyla oluşturuldu.
- [x] Stripe webhook idempotency `webhook_events` migration tablosuyla hizalandı.
...
METUnic panelinden (Hizmetler → piyasapilot.com → DNS ve Alan Adı Yönetimi → Yönet):
...
CNAME         api              piyasapilot.com            3600
...
**Doğrulama:**
```bash
dig piyasapilot.com A +short       # → sunucu IP
dig www.piyasapilot.com A +short   # → sunucu IP
curl -I https://piyasapilot.com    # → HTTP/2 200
```
...
Sertifika yolu: `/etc/letsencrypt/live/piyasapilot.com/`
Bu yol `docker/nginx.prod.conf`'ta kullanılıyor.

---

## 0.3 · `docker/nginx.prod.conf` — piyasapilot.com ile güncelle

Dosyadaki tüm `ornekdomain.com` → `piyasapilot.com` değiştir.
...
<meta property="og:url" content="https://piyasapilot.com">
<link rel="canonical" href="https://piyasapilot.com">
```
...
**URL:** `piyasapilot.com` (misafir kullanıcı)
...
**Detaylar:**
- Navbar: `piyasapilot.com` → logo sol, "Giriş Yap" + "Ücretsiz Başla" sağ
...
class StripeService:
    async def create_checkout_session(self, user_id: int, price_id: str,
        ...
        session = stripe.checkout.Session.create(
            ...
            mode="subscription",
            success_url="https://piyasapilot.com/payment/success?session_id={CHECKOUT_SESSION_ID}",
            cancel_url="https://piyasapilot.com/pricing",
...
> URL: `piyasapilot.com/admin` — sadece `role=admin`
...
- [x] Public route daha JS modülü yüklenmeden HTML/CSS seviyesinde terminal shell'i gizlesin; public sayfada kısa süreli topbar/sidebar parlaması olmasın.
...
- [x] `dailyLoss >= 0 ? dailyLossPct : -dailyLossPct` ifadesini kaldır; günlük K/Z yüzdesini yönlü değerle hesapla ve `formatPct()` ile tek işaret üret.
...
**Mobil WS bağlantısı:** `wss://piyasapilot.com/ws/quotes?token=<access_token>` (Bearer header WS'de çalışmaz).
...
```
[ ] piyasapilot.com HTTPS üzerinden açılıyor (TLS geçerli)
[ ] www.piyasapilot.com → piyasapilot.com'a yönleniyor (301)
...
Production için: `flutter build apk --dart-define=API_URL=https://piyasapilot.com`
...
# /etc/letsencrypt/live/piyasapilot.com/privkey.pem
...
```bash
curl https://piyasapilot.com/api/health
# → {"status": "ok", ...}

curl -o /dev/null -s -w "%{http_code}" https://piyasapilot.com
# → 200
```
...
```
Monitor type: HTTPS
URL: https://piyasapilot.com/api/health
Interval: 5 minutes
...
add_header Content-Security-Policy "
  ...
  connect-src 'self' https://piyasapilot.com wss://piyasapilot.com
              https://*.sentry.io;
...
**Çözüm:**
- Email doğrulama URL'i: `https://piyasapilot.com/verify?token=xxx`
...
- iOS: `apple-app-site-association` dosyası (AASA) → `piyasapilot.com/.well-known/apple-app-site-association`
...
export default function () {
  const r = http.get('https://piyasapilot.com/api/health');
...
2. piyasapilot.com domainini ekle
...
<script defer data-domain="piyasapilot.com"
...
### 16.4.5 · Status Page (piyasapilot.com/status)
...
- `piyasapilot.com/r/ENES42` linki paylaşılır
...
async function shareBacktest(result: BacktestResult): Promise<void> {
  ...
  // → https://piyasapilot.com/shared/abc123
...
**Public backtest sayfası:** `piyasapilot.com/shared/abc123` → giriş gerektirmez, misafir görebilir.
...
**Araç:** Blog için Astro veya Next.js (ayrı domain: `blog.piyasapilot.com`) veya Ghost.
...
- Kanal açıklamasında: "Ücretsiz kayıt: piyasapilot.com"
...
- Affiliate link: `piyasapilot.com?ref=PARTNER_CODE`
...
**Araç:** `piyasapilot.com/changelog` veya Headway (embed widget).
...
## Genel Değerlendirme: %61 Tamamlandı
...
| 2 — Kullanıcı Rol/Yetki | %100 | **%100** | Migration ve tek feature-gate kaynağı korunuyor; endpoint guard testleri Bölüm 3 altında doğrulandı |
| 3 — Backend Auth | %92 | **%92** | Register/login/me/session/logout/TOTP kapsamı mevcut; guard testleri ve mobil Bearer token akışı eklendi, canlı Google OAuth kaldı |
| 4 — Ekran Tasarımları | %96 | **%96** | Public/protected route ayrımı ve desktop+390px QA geçti; canlı entegrasyon edge-case'leri kaldı |
| 5 — Ödeme / Stripe | %75 | **%75** | Checkout/portal/webhook/idempotency kod kontratı hazır; canlı product/price/webhook dashboard testi kaldı |
| 6 — Admin Paneli | %88 | **%88** | Admin guard, kullanıcı/audit/veri kalite empty state ve yetkisiz UX tamam; canlı API metrikleri kaldı |
| 7 — Ödeme Sonrası / Plan | %90 | **%90** | Settings/payment success/portal/upgrade dili hazır; canlı Stripe uçtan uca doğrulama kaldı |
| 8 — i18n / Hukuki | %93 | **%93** | Public/auth/settings/payment/legal risk dili tutarlı; terminal içi tüm mikro metinlerin taşınması kaldı |
| 9 — UI Hataları | %100 | **%100** | WEB_UX_TEST_RAPORU blockerları ve 9.17–9.40 regresyon maddeleri frontend tarafında kapandı |
| 10 — Flutter Mobil | %55 | **%55** | Flutter iskeleti, ekranlar, API client, theme/routing/env ve README var; native store/push/signing kaldı |
| 11 — ClickHouse / Veri | %50 | **%50** | Şema/facade/badge kontratı hazır; canlı backfill, retention ve provider doğrulaması kaldı |
| 12 — CI/CD | %75 | **%75** | GitHub Actions test/build/e2e/docker/trivy olarak güncellendi; gerçek secrets/deploy runner kaldı |
| 13 — Monitoring | %70 | **%70** | Sentry/Grafana/Prometheus iskeleti hazır; canlı DSN/dashboard/alert kanalı kaldı |
| 14 — Kabul Testi | %82 | **%82** | Frontend, backend, Flutter ve desktop/mobile QA çalıştı; canlı domain/Stripe/Sentry kabulü kaldı |
| 15 — AWS | %45 | **%45** | Terraform scaffold ve deployment checklist hazır; canlı AWS/DNS/TLS/S3/IAM yapılmadı |
| 16 — Teknik Maddeler | %85 | **%85** | Analytics/PWA/skeleton/code-splitting ve test kontratı iyi durumda; status page/canlı monitoring kaldı |
| 17 — Büyüme / Roadmap | %55 | **%55** | Waitlist/referral/share kontratları hazır; blog/community/affiliate canlı kanal işleri kaldı |
...
1. **Stripe canlı ürün/price id** bağlı değil — para alınamıyor (Bölüm 5)
2. **AWS / sunucu** kurulmadı — domain canlıya çıkamıyor (Bölüm 15)
3. **Canlı DNS/TLS doğrulaması** yapılmadı — gerçek domain production kabulü eksik (Bölüm 0)
4. **ClickHouse/Redis veri platformu prod kabulü** tamamlanmadı (Bölüm 11)
5. **Mobil web ve Flutter mobil uygulama** geniş kullanıcı kitlesi için hâlâ ürünleşme istiyor (Bölüm 9.6 / Bölüm 10)
...
**Hafta 1 (Blocker'lar):**
- 9.5 / 9.7 / 9.17 / 9.28 blocker'ları yeniden QA ile doğrula
- Stripe canlı ürün/price id ve webhook secret'ı bağla
- Backend auth guard testlerini güncelle ve çalıştır

**Hafta 2 (UX Kalitesi):**
- 9.30–9.37 arası UI/teknik borç maddelerini regresyonla doğrula
...
- [ ] METUnic panelinde `piyasapilot.com` DNS yönetimini aç.
- [ ] `@` A kaydı: Elastic IP.
- [ ] `www` A kaydı: Elastic IP.
- [ ] `api` CNAME veya A kaydı: production host.
- [ ] Gerekiyorsa `api-mobile` A kaydı: Elastic IP.
- [ ] `dig piyasapilot.com A +short` ile IP doğrula.
- [ ] `dig www.piyasapilot.com A +short` ile IP doğrula.
...
- [ ] `www.piyasapilot.com` -> `piyasapilot.com` 301 yönlendirmesi çalışıyor mu kontrol et.
...
- [ ] Email linklerinin `https://piyasapilot.com` domainiyle açıldığını doğrula.
...
- [ ] Public status page kararı ver: `status.piyasapilot.com`.
...
- [ ] Destek e-postası oluştur: örn. `destek@piyasapilot.com`.
