...
const VALID_TIMEFRAMES: ReadonlySet<string> = new Set(['1m', '5m', '15m', '30m', '1h', '4h', '1d', '1w']);
...
function syncUrlState(): void {
  ...
  url.searchParams.set('tab', activeTab ?? 'chart');
  const sym = multiChart.getActivePaneSymbol()?.symbol;
  if (sym) url.searchParams.set('symbol', sym);
  url.searchParams.set('timeframe', multiChart.getActivePaneTimeframe());
  history.replaceState(null, '', url.toString());
}

function showTab(tab: string, persist = true, sync = true): void {
...
const _bootUrlSymbol = _bootUrlParams.get('symbol');
const _bootUrlTimeframe = _bootUrlParams.get('timeframe');
...
showTab(initialTab, false, false);
...
(function applyUrlParams(): void {
  const tfParam = _bootUrlTimeframe;
  if (isTimeframe(tfParam)) {
    multiChart.setActivePaneTimeframe(tfParam);
    dataEngine.setTimeframe(tfParam);
  }
  const symParam = _bootUrlSymbol;
  if (symParam) {
...
const startupSymbol = _bootUrlSymbol
  ? dataEngine.getSymbolInfo(_bootUrlSymbol) || {
      symbol: _bootUrlSymbol,
      name: _bootUrlSymbol,
      assetType: 'equity',
      group: 'BIST',
      currency: 'TRY',
    }
  : sidebar.getStartupSymbol();
if (!_bootUrlSymbol) void openSymbol(startupSymbol);
