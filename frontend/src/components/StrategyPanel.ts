...
export class StrategyPanel {
  ...
  setCandles(candles: OHLCV[], symbol?: string, interval?: Timeframe): void {
    ...
    this.lastRunKey = key;
    void this.runAnalysis({ silentGate: true });
  }
      ...
      <div class="strategy-wrap">
        ...
        <div class="strategy-topline">
          ...
          <div class="topline-actions">
            ...
            <span id="bt-quota-badge" class="bt-quota-badge" title="Günlük backtest kotası"></span>
  ...
  private async runAnalysis(opts: { silentGate?: boolean } = {}): Promise<void> {
    this.syncControls();
    if (this.runInFlight) {
  ...
  /** Backtest kota rozetini günceller — her tüketimden sonra çağrılır. */
  private _updateQuotaBadge(): void {
    const badge = this.container.querySelector<HTMLElement>('#bt-quota-badge');
    if (!badge) return;
    const remaining = planGate.getRemainingBacktests();
    const limit     = planGate.getBacktestLimit();
    if (limit === Infinity) {
    ...
    badge.className = 'bt-quota-badge' +
      (remaining === 0 ? ' bt-quota-empty' : remaining <= 2 ? ' bt-quota-low' : '');
    badge.title = `Bugün ${limit - remaining} / ${limit} backtest kullanıldı`;
  }
