...
import { DataQualityBadge } from './DataQualityBadge.js';
...
export class ChartPanel {
  ...
  private qualityBadge: DataQualityBadge | null = null;
  ...
  private readonly timeframeLabels: Record<Timeframe, string> = {
    '1m': TR.TF_1M,
    '5m': TR.TF_5M,
    '15m': TR.TF_15M,
    '30m': TR.TF_30M,
  ...
  private buildDOM(): void {
    ...
    // Veri kalite rozeti — kontrol çubuğunun sağına eklenir
    const badgeWrap = document.createElement('span');
    badgeWrap.className = 'chart-dqb-wrap';
    badgeWrap.style.cssText = 'position:absolute;top:6px;right:12px;z-index:10;';
  ...
  private controlsHTML(): string {
    const tfs = Object.entries(this.timeframeLabels)
  ...
  setData(candles: OHLCV[], options: ChartViewOptions = {}): void {
    ...
    this.updateTimeframeControls(this.lastTimeframe);
  ...
  /**
   * Veri kalitesi rozetini günceller.
   * HistoricalLoader'dan gelen DataTruth nesnesiyle çağrılmalı.
   */
  updateDataTruth(truth: DataTruth): void {
    this.qualityBadge?.update(truth);
  }

  /** Rozeti sıfırlar (yeni sembol yüklenirken). */
  clearDataTruth(): void {
    this.qualityBadge?.setStatus('unknown', 'Yükleniyor…');
  }
  ...
  private updateTimeframeControls(timeframe: Timeframe | null): void {
    if (!timeframe) return;
    const controls = this.container.querySelector<HTMLElement>('.chart-controls');
    if (!controls) return;

    controls.querySelectorAll<HTMLElement>('.tf-btn').forEach(button => {
      button.classList.toggle('active', button.dataset['tf'] === timeframe);
    });

    const triggerLabel = controls
      .querySelector<HTMLElement>('.tf-btn')
      ?.closest('.tool-cluster')
      ?.querySelector<HTMLElement>('.tool-trigger b');
    if (triggerLabel) triggerLabel.textContent = this.timeframeLabels[timeframe] ?? timeframe;
  }
