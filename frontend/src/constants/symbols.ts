...
export const BIST100_EXTRA: SymbolInfo[] = [
  ...
  { symbol: 'AKFYE.IS', name: 'Akfen Yenilenebilir Enerji', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  ...
  { symbol: 'ALFAS.IS', name: 'Alfa Solar Enerji', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  { symbol: 'ASTOR.IS', name: 'Astor Enerji', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  ...
  { symbol: 'BIENY.IS', name: 'Bien Yapı', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  { symbol: 'BRISA.IS', name: 'Brisa', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  { symbol: 'BRSAN.IS', name: 'Borusan Mannesmann', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  ...
  { symbol: 'GWIND.IS', name: 'Galata Wind Enerji', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  ...
  { symbol: 'KAREL.IS', name: 'Karel Elektronik', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  { symbol: 'KARSN.IS', name: 'Karsan Otomotiv', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  { symbol: 'KCAER.IS', name: 'Karsu Tekstil', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  { symbol: 'KMPUR.IS', name: 'Kimpur', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
  ...
  { symbol: 'PAPIL.IS', name: 'Papilon Savunma', assetType: 'equity', group: 'BIST 100', currency: 'TRY' },
...
// ─── Backend'den sembol evreni yükleme ───────────────────────────────────────
...
let _cachedApiSymbols: SymbolInfo[] | null = null;

/**
 * Backend /api/symbols endpoint'inden sembol listesini çeker.
 * Başarılı olursa sonucu önbelleğe alır; hata olursa LOCAL ALL_SYMBOLS döner.
 *
 * Kullanım:
 *   const symbols = await fetchSymbolsFromApi();
 */
export async function fetchSymbolsFromApi(): Promise<SymbolInfo[]> {
  if (_cachedApiSymbols) return _cachedApiSymbols;
  try {
    const res = await fetch('/api/symbols');
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json() as { symbols: ApiSymbol[] };
    const mapped: SymbolInfo[] = data.symbols.map(s => ({
      symbol:    s.symbol,
      name:      s.name,
      assetType: s.asset_type as SymbolInfo['assetType'],
      group:     s.group,
      currency:  s.currency,
    }));
    _cachedApiSymbols = mapped;
    return mapped;
  } catch {
    // API erişilemiyorsa yerel sabitlere geri dön
    return ALL_SYMBOLS;
  }
}

/** Önbelleği temizler (oturum değişiminde kullanılabilir). */
export function clearSymbolCache(): void {
  _cachedApiSymbols = null;
}
