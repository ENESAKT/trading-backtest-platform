...
## Flutter / Mobil
- 2026-05-26 Flutter analyze CI hatalarının kök nedenleri: (1) Model-widget tip uyumsuzluğu — `DataQualityBadge` widget'ı `DataTruth` (non-nullable) beklerken, tüm modeller (`SymbolSnapshot.dataTruth`, `SignalEvidence.dataTruth` vb.) `DataTruth?` (nullable) döndürüyordu. Çözüm: Widget'ı nullable kabul edecek şekilde güncelleyip `null` durumunda `SizedBox.shrink()` döndürmek. (2) `DataQualityStatus` enum'unda `good`/`poor` sabitleri tanımlı değildi; doğru değerler `ok`/`blocked`. (3) `TechnicalRatingCard` widget'ı `String` alırken ekranlardan `TechnicalRating` enum gönderiliyordu. (4) `ScreenerFilterRule` parametresi `operator` iken ekranda `op` kullanılmıştı. (5) `ScreenerRunResponse`'da `results` getter'ı yoktu, doğru alan adı `rows`. (6) `assets/images/` dizini pubspec.yaml'da tanımlı ama fiziksel olarak oluşturulmamıştı → `.gitkeep` ile çözüldü.
- 2026-05-27 `pubspec.yaml`'da `fonts/Roboto-Regular.ttf` ve `Roboto-Bold.ttf` tanımlıyken fiziksel dosyalar mevcut değildi → CI "unable to locate asset entry" hatası. Çözüm: Flutter `uses-material-design: true` zaten Roboto'yu varsayılan sağladığı için font tanımları tamamen kaldırıldı.
- 2026-05-27 Flutter SDK güncellemelerinde `CardTheme(...)` doğrudan `ThemeData.copyWith(cardTheme:)` parametresine geçirilemez; yeni API `CardThemeData(...)` tipini bekler. Aynı pattern `NavigationBarThemeData` gibi diğer theme sınıfları için de geçerlidir.
