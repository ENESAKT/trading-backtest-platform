...
- Geçersiz mode reddedilmeli (STR-3 FIX)
- Boş DataFrame yazılmamalı
- Schema uyumu
- Parametreli sorgular (STR-5 FIX)
- Atomic write (STR-8 FIX)
"""
...
class TestStorageWriteRead:
    ...
    def test_write_and_read_back(self, storage, sample_df):
        ...
        rows = storage.write_symbol_data(
            sample_df, "THYAO", mode="overwrite"
        )
        assert rows == 3
    ...
    def test_write_preserves_close_values(
        self, storage, sample_df
    ):
        """Close fiyatları yazılıp okunurken bozulmamalı."""
        storage.write_symbol_data(
            sample_df, "THYAO", mode="overwrite"
        )
        read_back = storage.read_symbol("THYAO")
        ...
        assert original_closes == pytest.approx(
            read_closes, abs=0.01
        )
    ...
    def test_read_nonexistent_symbol(self, storage):
        """Var olmayan sembol okunursa boş DataFrame."""
...
class TestStorageMode:
    ...
    def test_invalid_mode_rejected(
        ...
        """STR-3 FIX: Geçersiz mode ValueError fırlatmalı."""
        with pytest.raises(ValueError, match="Geçersiz"):
            storage.write_symbol_data(
                sample_df, "MODE_TEST", mode="nonsense"
            )
    ...
    def test_overwrite_replaces_all(
        ...
        storage.write_symbol_data(
            sample_df, "OVR", mode="overwrite"
        )
        storage.write_symbol_data(
            sample_df.head(1), "OVR", mode="overwrite"
        )
    ...
    def test_append_adds_rows(self, storage, sample_df):
        ...
        storage.write_symbol_data(
            sample_df.head(2), "APP", mode="overwrite"
        )
        ...
        storage.write_symbol_data(
            new_df, "APP", mode="append"
        )
        result = storage.read_symbol("APP")
        assert len(result) == 3

    def test_bulk_write_invalid_mode_rejected(
        self, storage, sample_df
    ):
        """STR-3 FIX: write_bulk'ta da geçersiz mode."""
        with pytest.raises(ValueError, match="Geçersiz"):
            storage.write_bulk(
                {"TEST": sample_df}, mode="invalid"
            )
...
class TestStorageSymbolOps:
    ...
    def test_symbol_exists(self, storage, sample_df):
        ...
        storage.write_symbol_data(
            sample_df, "EXISTS_TEST", mode="overwrite"
        )
    ...
    def test_get_last_date(self, storage, sample_df):
        ...
        storage.write_symbol_data(
            sample_df, "DATE_TEST", mode="overwrite"
        )
        last = storage.get_last_date("DATE_TEST")
        assert last is not None


class TestStorageColumnValidation:
    """STR-5 FIX: Sütun allow-list testleri."""

    def test_valid_columns_accepted(
        self, storage, sample_df
    ):
        """Geçerli sütun isimleri kabul edilmeli."""
        storage.write_symbol_data(
            sample_df, "COL_TEST", mode="overwrite"
        )
        result = storage.read_symbol(
            "COL_TEST", columns=["close", "volume"]
        )
    ...
    def test_invalid_columns_rejected(
        ...
        """Geçersiz sütun isimleri ValueError fırlatmalı."""
        storage.write_symbol_data(
            sample_df, "COL_TEST2", mode="overwrite"
        )
        with pytest.raises(ValueError, match="Geçersiz"):
            storage.read_symbol(
                "COL_TEST2", columns=["hacked_col"]
            )
